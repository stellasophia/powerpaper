...
For power analysis, we now need to change the underlying reality: Instead of assuming the null hypothesis to be true, we now assume that the alternative hypothesis is true in that we condition on the alternative hypothesis. This is represented by the right columns in both tables. Again, the significance test has two possible outcomes: If the test is negative, it is a false negative (also called type II-error); if the null hypothesis is rejected, it is a true positive (see Table \ref{tab:decerr}). The corresponding probabilities are called $\beta$ (the probability of a Type II-error), and 1-$\beta$ (the probability of a true positive), as can be seen in Tabel \ref{tab:errorprob}. 1-$\beta$ is called the power of the test. Thus, power gives an answer to the question: What is the probability that a statistical test is significant, given that the effect exists (probability of a true positive). Regarding our example, the statistical power indicates the probability that the test is significant, given that the treatment does increase concentration performance.
...
When the $\chi^2$ value is directly used for sample size planning, we need the exact specification of two correlation matrices: One under the null hypothesis and one under the alternative hypothesis. They are the model implied variance covariance matrices under the null and the alternative model respectively. The alternative model is the one that we assume is true in the population and the null model contains one restriction to that model, i.e. one parameter is fixed to a different value.
The idea of the sample size planning is then to determine the required sample size to detect the given misspecification.

In order to obtain the variance covariance matrix under the alternative hypothesis, we can simply use the \texttt{sem} function from the \texttt{lavaan} package \citep{lavaan}. By only specifying a model and no data to this function, we can obtain the model implied variance covariance matrix. We are going to call this matrix the alternative variance covariance matrix and we are going to use it as the empirical matrix for the next step. As an alternative model, we are going to use a two factor model with four items for each factor and a factor intercorrelation of .9. All loadings also have to be specified:
...
The restriction (here: misspecification) in the null model will be a factor intercorrelation of 1.00. All other parameters can be freely estimated, since the factor intercorrelation is the only parameter we are interested in:
<<santorra h1>>=
 model.null <- '
            f1 =~ NA*x1 + x2 + x3 + x4
            f2 =~ x5 + x6 + x7 + x8
...
We need 860 people to detect a factor inter-correlation of .90 between f1 and f2 with a power of 80\%.

\subsubsection{RMSEA based approach}

For sample size planning with the RMSEA the fact is used that the RMSEA is a function of the fit function $F_{ML}$ that is used for calculating the $\chi^2$ value. Hence, the RMSEA also can be transformed into the noncentrality parameter for the $\chi^2$ distribution under the alternative hypothesis. Common conventions for the RMSEA say, that under the null hypothesis that the model fits the data, the RMSEA should be less than .05 \citep[e.g.][]{browne1992alternative}:
\[H_0: RMSEA < .05 \].
Browne and Cudeck \citet{browne1992alternative} suggested that values between .05 and .08 indicate a fair fit and values between .08 and .10 were declared as a poor fit, whereas MacCallum et al. \citet{maccallum1996power} consider values between .08 and .10 to indicate a mediocre fit.
...
As was the case with multilevel models, it is possible to use the results of simulations to estimate the power of a statistical test for a given sample size and a given pair of null and alternative hypotheses. The overall procedure is identical to that of simulation-based approaches for IRT models.

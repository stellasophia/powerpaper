...
\title{A gentle introduction to Power Analysis \\ with an outlook on some recent applications}
...
\SweaveOpts{concordance=TRUE}
	...
	\stella{Dieses Kapitel muss stark gekürzt werden. Überlegen, ob wir es nicht sogar ganz entfernen und mit What is statistical power anfangen.}
	
	We have already introduced statistical tests in the previous chapter. The focus of this chapter will be on power analysis and sample size calculation. We will learn how low power has the consequence that research results produce less knowledge gain. 
	
	Say that in some scientific study our goal is to investigate whether a treatment increases performance in a concentration test. In order to plan this study, it is important to know how many people we need. If the sample is too small, we do not know whether a non-significant result is due to a lack of a treatment effect or due to a too small sample size. We will see that even in case of a significant test result, the knowledge gain is reduced. In this chapter, therefore, the basics of statistical power analysis are introduced and it will be explained how it is used for sample size planning.
	...
	Probabilities can be visualized as areas. For visualizing the probabilities $\alpha$, $\beta$, and 1-$\beta$, we use probability distributions. The area under the probability distribution for a given interval represents the probability that the values in this interval occur, given the respective hypothesis. Which probability distribution we use, depends on the test statistic that in turn depends on the statistical test we use. 
	...
	\begin{figure}[ht]
		\centering
		\includegraphics[width = 0.8\textwidth]{plots/distribution_plot.png}
	...
	\begin{enumerate}
		...
		3. Choose the expected effect under the alternative hypothesis, and thereby determine the distribution of the test statistic when the alternative hypothesis is true.
	...
	The PPV, of course, is highly dependent on the a priori assumed probability of a real effect, that we set to 30\% in this example. The higher the a priori probability of an effect, the higher the PPV. 
	...
	In the following, we will discuss what factors have an influence on the power. Some of them are theoretical decisions that have to be set by the researcher (as for example the $\alpha$ level and the expected effect under the alternative hypothesis). And some are characteristics of the study design, that can if ever only be controlled before data collection.
	...
	First, we will take a look at determinants of the power, that the researcher decides on independent of the study design and data collection. Those are:
	...
	\begin{figure}[ht]
		\centering
		\includegraphics[width = 0.8\textwidth]{plots/power_alpha.png}
		\caption{How the power ($1-\beta$) changes depending on $\alpha$}
		\label{fig:pwralpha}
	\end{figure}
	From a theoretical perspective, increasing $\alpha$ means that we are less strict in our decision, because we allow the propability of a false positive to be higher. By shifting the critical value to the left, we increase the overall chance of rejeting the null hypothesis. Consequently, the propability of rejecting the null hypothesis given that it is true, also increases. Therefore, increasing $\alpha$ results in an increase of $1-\beta$. 
	...
	To understand what happens when the expected effect size under the alternative hypothesis increases, let us take look at the distribution plot in Figure \ref{fig:pwreffect}.
	\begin{figure}[ht]
		\centering
		\includegraphics[width = 0.8\textwidth]{plots/power_effectsize.png}
	...
	Furthermore, there are a number of influences on power, that are already determined by the design of the study. Considering them carefully before conducting the study, they can help improve the power without having to change parameters that are supposed to be theoretical decisions. 
	...
	Unlike the standard deviation $\sigma$, the sample size $n$ is something that can be manipulated by the researcher directly. 
	...
	\begin{figure}[ht]
		\centering
		%  \includegraphics[width = 0.8\textwidth]{plots/effectOfSampleSizeI.png}
		% \includegraphics[width = 0.8\textwidth]{plots/effectOfSampleSizeII.png}
	...
	In a similar way, required sample sizes can be computed for other parametric and exact tests, for example $z$-test for correlations, $z$-tests and $t$-tests for mean comparisons, F-tests for analyses of variance and regression models, and $t$-tests for regression coefficients. 
	
	Calculations can be done with statistical software, such as G*Power, that can be downloaded for free from \url{http://www.gpower.hhu.de}, or in R with the package \texttt{pwr}, that we will use in our practical examples in the following chapters.
	
	However, for the calculation of power and therefore also for the sample size calculation, we always need the distribution of the test statistic under the alternative hypothesis. Unfortunately, there are statistical tests for which this distribution cannot be determined that easily. In such scenarios, a different procedure has to be used.
	
	One approach is to determine the influence of $n$ on the standard error of the effect estimator. Maximum likelihood estimators are asymptotically normally distributed, and thus we can use a $z$-test with a test statistic of the form $\frac{\textrm{effect size}}{\textrm{standard error}}$.
	
	The more common approach in more complex scenarios though, is to use simulation studies to determine the power of different sizes of $n$. This means that many different samples are randomly created for each size of $n$. They all have the same true effect size, that, of course, still has to be set by the researcher. Then, the statistical test of interest is run on each of them, and the number of significant test results is counted. The proportion of significant test results of the total number of samples is the power. 
	
	Let us use the $z$-test from above as an example. For illustration purposes, we start with an arbitrarily chosen sample size of $n$=25 and generate 25 random normally distributed numbers (e.g. using **rnorm**) with mean = 0.3 and sd = 1. For this sample, we then calculate a confidence interval with lower bound $\hat{x}_i - 1.645 \cdot se(\hat{x}_i)$. We can repeat this procedure let's say 5000 times (N = 5000) and check how many of the lower bounds of the confidence intervals are above 0 (meaning that the test is significant). The percentage of significant test results is an estimate of the power. Using the R code below with the given seed, we get an estimated power of 0.45.
	
We start by setting the parameters for the simulation, where N is the number of repetitions. Then, we generate N repetitions and calculate the lower bound of the confindence interval (CI). With that, we can calculate the rate of significant results
	<<simulation1>>=
...
@
	...
	for (j in 1:length(n)){
		...
		ll[[j]] <- rep(NA,N)
		for(i in 1:N){
			ll[[j]][i] <- mean(x[,i]) - 1.645 * (sd(x[,i])/sqrt(n[j]))
		}
	}
...
<<power norm>>=
	pwr.norm.test(d = 0.3, n = NULL, sig.level = 0.05, power = 0.8, alternative = "greater")
@
	...
	\subsection{Determining $n$ for ANOVA and regression models}
...
@
	...
	Please note that when calculating $n$ for a two-way ANOVA in G*Power, the total number of groups has to be specified that however does not have an influence on the resulting $n$ and also just like for the one-way ANOVA, $f$ is used as an effect size measure. 
	
	\subsubsection{Repeated measures ANOVA}
	
	The usual formula for calculating the effect size of a repeated meausures ANOVA is
	...
	When using G*Power for the power analysis of a repeated measures ANOVA, it has to be noted that G*Power knows different ways of calculating the effect size. In case the above formula is used, the settings have to be changend to "as in SPSS". Then, the correlation between measures does not have to be specified explicitly.
	
	Power analysis for repeated measures ANOVA is not possible with the \texttt{pwr} package. 
...
\begin{enumerate}
		\item $\beta_z$: Is the slope significantly different from 0?
		\item $\rho^2$: omnibus test: Is $R^2$ significantly different from 0?
\end{enumerate}
	
	
In the simple linear regression, those two are equivalent, except for the fact that the first hypothesis is tested with a t-test and the second with an F-test and therefore only the first hypothesis can be tested in a one-sided test.
...
@
	...
	In regressions with more than one predictor, power analysis for the regression coefficient $\beta_z$ gets tricky when those predictors are correlated. With collinear predictors, the standard error of the estimator $\beta_z$ and therefore also the power of $\beta_z$ depend on the size of the collinearity - the higher the collinearity, the higher the power.
	
	One solution of course would be to only use uncorrelated predictors, which is highly unlikely. And the other solution is to use the partial $r^2$ as effect size measure that is independent of the size of the collinearity.
	There are thus two possible effect size measures for which power analysis for multiple regression can be done in an analytical way:
...
@
	...
	\subsection{Determining $n$ for IRT models}
	...
	Zimmer, Draxler and Debelak (ref) suggested a method for an analytical power analysis that can also be used for other IRT models that can only be estimated with marginal maximum likelihood (MML) estimation. 
	...
	Another approach for power analysis for IRT models, is to use a sampling-based method (Gustadisegni et al., 2021; Gudicha et al., 2017; Zimmer et al., 2022), that is particularly recommendable with a large number of items, where the analytical procedures get very time consuming.
	\stella{RUDI: Könntest du hier beschreiben, wie die Verfahren von euch funktionieren?}
	
	\subsection{Determining $n$ for Multilevel models}
	
	\begin{itemize}
		\item Very few analytical procedures
		\item problem: ?
		\item Most applications for power analysis for multilevel models focus on simulation studies. 
	\end{itemize}
	
	
	One problem in power analysis for multilevel models is that significance tests are not as straightforward as they are in regression models. Three different types of significance tests are available:
	
	\begin{enumerate}
		\item Testing random effects with an LR test
...
@
	...
	\end{itemize}
	\subsection{Determining $n$ for Structural equation models (SEM)}
	
	Main goal: Comparison of model implied variance covariance matrix with empirical variance covariance matrix
	
	Commonly used measure for calculating this difference: $F_{ML}$ value:
...
It is not so clear, how to determine the required power, because there are no established guidelines. If there is a theretical reason for a particular power, this would be the first choice. If not, researchers typically choose one of the most commonly used values that are 80%, 90%, and 95%.

The next section addresses how we can determine the expected effect size.

### **5.1. How to determine the expected effect**
...
Therefore, the most common way in practice is to base power analyses on published effect sizes, that are supposed to reflect the underlying true effect. Even though this aussumption seems very straightforward, it is in fact very problematic. Replication studies have shown that 83% of all published correlations are higher than in the replication, the mean correlation over all original studies being 0.40 and over the replications being 0.20 [@open2015estimating]. Publication bias, which refers to the problem that only statistically significant test results are published, which in turn report higer effect sizes, are one reason the reported effect sizes are likely to be overestimated. For estimation of the true effect size, all studies (also those with non-significant effects) have to be considered. Franco and colleagues [-@franco16] could show that reported effect sizes are twice as large as unreported effect sizes. Such a difference dramatically changes the sample size needed for a chosen power. In the literature therefore it is mostly suggested to divide the reported effect sizes by two and use that value for the sample size calculation. It has also been suggested to use the lower end of a 60% confidence interval around the reported effect sizes ["Safeguard Power" by @perugini14]. 
...
## **6. Sample size calculation for more complex models**

In Section 1, we have seen that for the calculation of power and therefore also for the sample size calculation in Section 4, we need the distribution of the test statistic under the alternative hypothesis. Unfortunately, there are statistical tests for which this distribution cannot be determined that easily. In such scenarios, a different procedure has to be used.

One approach is to determine the influence of $n$ on the standard error of the effect estimator. Maximum likelihood estimators are asymptotically normally distributed, and thus we can use a $z$-test with a test statistic of the form $\frac{\textrm{effect size}}{\textrm{standard error}}$.

The other approach is to use simulation studies to determine the power of different sizes of $n$. This means that many different samples are randomly created for each size of $n$. They all have the same true effect size, that, of course, still has to be set by the researcher. Then, the statistical test of interest is run on each of them, and the number of significant test results is counted. The proportion of significant test results of the total number of samples is the power. 

Sometimes, in more complex models, there are different effect sizes of interest. In this case, one option is to determine the power for the smallest expected effect size. This results in the highest required $n$. The other option is to determine the power for one pre defined main target effect.

The advantage of simulation studies for power calculations is, that it can be done for any type of analysis. Even a combination of questions can be examined as, for example, the power for jointly testing all effects derived from a certain theory. Additionally, it is possible to analyze the influence of other factors, such as violations of the assumption of normality. 

The problem, however, of power analysis for very complex models, such as multilevel models, is, that everything that might have an influence (e.g., all parameters of the model) has to be prespecified, which can involve a lot of decisions that are usually difficult to make.


## **Solutions**

**1.**
The investigator can increase the power by increasing the sample size, using a within design instead of between design, and by using highly reliable measures (low measurement error), which decreases the standard deviation of the target measure.

**2.** 

```{r}
pwr.norm.test(d = 0.3, n = 25, sig.level = 0.05, power = NULL, alternative = "greater")
```

The power is 44.24% and thus much lower than in the example. This is because smaller effect sizes have less power, meaning they are much less likely to be detected than high effect sizes.
...
To achieve a power of 80% for this effect size, 69 subjects are needed.

**4.**


```{r}
pwr.norm.test(d = 0.3, n = 25, sig.level = 0.05, power = NULL, alternative = "two.sided")
```

The power decreases to 32.30%.

**5.**


```{r}
pwr.t.test(type="paired", d=0.4, n=NULL, sig.level = 0.05, power = 0.9, alternative="greater")
```

55 subjects have to be tested.

**6.**


```{r}
pwr.t.test(type="paired", d=NULL, n=10, sig.level = 0.05, power = 0.9, alternative="greater")
```

An effect of $\delta$ = 0.4 is needed to achieve a power of 90% with $n$=10.
